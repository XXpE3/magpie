...
describe('DebateOrchestrator', () => {
  ...
  it('marks false positives as not publishable during verification', async () => {
    const reviewer: Reviewer = {
      id: 'reviewer-1',
    ...
    const summarizer: Reviewer = {
      ...
      provider: createMockProvider('s', [
        '```json\n{"issues":[{"severity":"high","category":"security","file":"src/auth.ts","line":42,"title":"SQL injection risk","description":"Query concatenates user input"}]}\n```',
        '```json\n{"verified":[{"index":0,"status":"false_positive","severity":"nitpick","reason":"Caller already parameterizes the query","evidence":"src/auth.ts:40 uses bound parameters"}]}\n```'
      ]),
      systemPrompt: 'You are a summarizer'
    }
    const analyzer: Reviewer = {
    ...
    expect(result.parsedIssues).toHaveLength(1)
    expect(result.parsedIssues![0].severity).toBe('nitpick')
    expect(result.parsedIssues![0].verification).toEqual({
      status: 'false_positive',
      severity: 'nitpick',
      reason: 'Caller already parameterizes the query',
      evidence: 'src/auth.ts:40 uses bound parameters',
    })
